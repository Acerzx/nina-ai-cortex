...
Использование:
    ...
    # Периодическая проверка (из BackgroundTaskManager)
    predictions = await predictive_hal.check_all()

    # Конкретные предсказания
    guider_pred = await predictive_hal.predict_guider_failure()
    focus_pred = await predictive_hal.predict_focus_drift()
"""
...
class Prediction:
    ...
    prediction_type: str  # guider_failure, focus_drift, ...
...
class PredictiveHAL:
    ...
    Проактивно анализирует тренды метрик и предсказывает:
    - Потерю гидирования (по тренду RMS)
    - Дрейф фокуса (по корреляции температуры и HFR)
    - Перегрев оборудования
    - Деградацию качества изображения

    Интегрируется с:
    - Watcher (для генерации PREDICTIVE_ALERT)
    - Guardian (для упреждающих действий)
    - Strategist (для адаптивных оптимизаций)
    """

    # Размеры окон анализа (в точках истории)
    WINDOW_SHORT = 10  # ~30 секунд при 3с интервале
    WINDOW_MEDIUM = 20  # ~1 минута
    WINDOW_LONG = 40  # ~2 минуты

    # Горизонт предсказания (минуты вперёд)
    ...
    def __init__(self):
        ...
        # История предсказаний (для избежания спама)
        self._recent_predictions: Dict[str, datetime] = {}
        self._prediction_cooldown_seconds = (
            300  # 5 минут между одинаковыми предсказаниями
        )

        # Статистика
        self._stats = {
            "total_predictions": 0,
            "accurate_predictions": 0,  # TODO: hindsight verification
            "false_positives": 0,
            "checks_performed": 0,
        }
    ...
    def _load_thresholds(self) -> Dict[str, float]:
        """Загружает динамические пороги confidence."""
        defaults = {
            "CRITICAL": 0.95,
            "MEDIUM": 0.85,
            "LOW": 0.70,
    ...
    async def predict_guider_failure(
        ...
        Алгоритм:
        1. Берём историю RMS за последние window_minutes
        2. Строим линейную регрессию (RMS vs time)
        3. Экстраполируем на PREDICTION_HORIZON_MINUTES вперёд
        4. Если предсказанный RMS > критического порога → предсказание

        Args:
            window_minutes: Окно анализа (минуты)
        ...
        # Линейная регрессия для RMS RA
        trend_ra, intercept_ra = self._linear_regression(recent_ra)

        # Текущее значение
        current_ra = recent_ra[-1]

        # Экстраполяция на PREDICTION_HORIZON_MINUTES
        # Предполагаем ~1 точка в 3 секунды (из InfluxDB poll interval)
        points_per_minute = 20.0  # 60s / 3s
        ...
        # Критический порог из Guardian thresholds
        from app.core.config import settings

        critical_rms = getattr(settings.thresholds.guardian, "rms_recalibration", 3.0)

        # Проверяем, превысит ли RMS критический порог
    ...
    async def predict_focus_drift(self) -> Optional[Prediction]:
        ...
        Алгоритм:
        1. Проверяем корреляцию между температурой и HFR за последнее время
        2. Если корреляция сильная (|r| > 0.7) и температура меняется → дрейф
        3. Extrapolate HFR based on temperature trend
        ...
        if abs(correlation) < 0.6:
            # Слабая корреляция — дрейф маловероятен
            return None
        ...
        # Предсказываем HFR через PREDICTION_HORIZON_MINUTES
        points_per_minute = 20.0
        future_points = int(self.PREDICTION_HORIZON_MINUTES * points_per_minute)
        predicted_hfr = hfr_intercept + hfr_trend * (len(recent_hfr) + future_points)

        # Порог деградации HFR
        from app.core.config import settings

        hfr_target = getattr(settings.thresholds.strategist, "hfr_target", 2.5)
        degradation_threshold = hfr_target * 1.3  # 30% рост

        if (
            predicted_hfr > degradation_threshold
            and current_hfr < degradation_threshold
        ):
            # Время до превышения порога
            if hfr_trend > 0:
                points_to_threshold = (
            ...
            # Confidence: сила корреляции + сила тренда
            ...
            return Prediction(
                ...
                recommended_action=(
                    ...
                    if time_to_event
                    else f"Температурный дрейф фокуса: рекомендуется автофокус"
                ),
                action_type=ActionType.MEDIUM,
                evidence={
    ...
    async def predict_quality_degradation(self) -> Optional[Prediction]:
        ...
        Анализирует тренды HFR и FWHM независимо от температуры.
        """
        hfr_history = observatory_state.history.hfr
        ...
        # Если тренд положительный и предсказание > 150% от целевого
        if (
            trend > 0
    ...
    async def predict_camera_overheat(self) -> Optional[Prediction]:
        ...
        # Порог тревоги (например, -5°C — камера плохо охлаждается)
        warning_temp = -5.0
    ...
    def _should_publish(self, prediction: Prediction) -> bool:
        ...
        # Только если confidence выше порога для данного типа действия
