...
ИСПРАВЛЕНО (audit 7.2): Все магические числа вынесены в
settings.thresholds.watcher для централизованной настройки.
"""
...
class WatcherAgent(BaseAgent):
    ...
    - Все пороговые значения вынесены в settings.thresholds.watcher
    ...
    def __init__(self):
        ...
        thresholds_cfg = settings.thresholds.watcher
        self.thresholds = {
            "hfr_increase_percent": thresholds_cfg.hfr_increase_percent,
            "fwhm_increase_percent": thresholds_cfg.fwhm_increase_percent,
            "rms_ra_critical": thresholds_cfg.rms_ra_critical,
            "rms_dec_critical": thresholds_cfg.rms_dec_critical,
            "temperature_deviation": thresholds_cfg.temperature_deviation,
            "wind_speed_warning": thresholds_cfg.wind_speed_warning,
            "wind_gust_critical": thresholds_cfg.wind_gust_critical,
            "z_score_threshold": thresholds_cfg.z_score_threshold,
            "min_history_points": thresholds_cfg.min_history_points,
        }

        # История последних аномалий (для избежания спама)
        self._recent_anomalies: Dict[str, datetime] = {}
        self._anomaly_cooldown_seconds = thresholds_cfg.anomaly_cooldown_seconds
    ...
    async def _check_hfr_trend(self) -> Optional[AnomalyReport]:
        ...
        # Для малых выборок — упрощенная детекция
        if len(history) < 5:
    ...
    async def _check_rms_metrics(self) -> Optional[AnomalyReport]:
        ...
        if rms_dec and rms_dec > rms_dec_threshold:
            history = observatory_state.history.rms_dec
            recent = history[-3:] if len(history) >= 3 else []
            if len(recent) >= 3 and all(v > rms_dec_threshold for v in recent):
    ...
    # ========================================================================
    # Публичные методы для Orchestrator
    # ========================================================================
