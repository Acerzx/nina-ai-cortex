...
class WatcherAgent(BaseAgent):
    ...
    async def _on_log_event(self, data: Dict[str, Any]):
        ...
        if event_type == "safety_unsafe":
            await self._generate_critical_alert(
                "Safety Monitor перешел в UNSAFE", data
            )
    ...
    async def _check_hfr_trend(self) -> Optional[AnomalyReport]:
        ...
        if len(history) >= 3:
            logger.debug(
                f"   Last 5 values: "
                f"{history[-5:] if len(history) >= 5 else history}"
            )
        ...
        recent = history[-5:]
        baseline = (
            history[:-5] if len(history) > 5 else history[: len(history) // 2]
        )
        if not baseline:
            return None
        ...
        if increase_percent > hfr_threshold:
            ...
            std = (
                (
                    sum((x - baseline_mean) ** 2 for x in baseline)
                    / len(baseline)
                )
                ** 0.5
                if len(baseline) > 1
            ...
            z_score = (
                abs(current_mean - baseline_mean) / std if std > 0 else 0
            )
