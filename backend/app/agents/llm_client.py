...
class LLMClient:
    ...
    def is_available(self) -> bool:
        ...
        ИСПРАВЛЕНО (С-13): Безопасная проверка наличия event loop
        через try/except RuntimeError.
        ...
        if self._availability_cache is not None:
            ...
            # ИСПРАВЛЕНО (С-13): Безопасная проверка event loop
            ...
            try:
                ...
                # или при вызове из синхронного контекста
                pass
            ...
            if loop is not None:
                try:
                    ...
                    # Loop закрыт или недоступен — пропускаем фоновое обновление
                    pass
        ...
        return bool(
            llm_provider.config.primary_model or llm_provider.config.fallback_model
        )
