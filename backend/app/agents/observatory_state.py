...
- Удалены неиспользуемые методы:
  - clear_resolved_alerts() — не вызывается нигде
  - get_session_summary() — дублирует get_full_state()
"""
...
class ObservatoryState:
    ...
    ИСПРАВЛЕНО (v3):
    - Лимиты истории и алертов читаются из settings.metrics
    - Удалены неиспользуемые методы clear_resolved_alerts() и get_session_summary()
    """
    ...
    def __init__(self):
        ...
        # === ИСПРАВЛЕНО (v3): Лимиты из settings.metrics ===
        ...
        # История действий AI (для объяснимости) — используем deque для O(1)
    ...
    # ========================================================================
    # Логика выбора активного источника
    # ========================================================================
    ...
    async def _on_alert(self, data: Dict[str, Any]):
        ...
        # ИСПРАВЛЕНО (v3): лимит из settings.metrics.active_alerts_max
    ...
    def _append_history(self, metric: str, value: Optional[float]):
        ...
        ИСПРАВЛЕНО (v3): лимит читается из settings.metrics.history_max_points.
        """
        if value is None:
            return
    ...
    def is_metric_degrading(self, metric: str, threshold_percent: float = 20.0) -> bool:
        """Проверяет, деградирует ли метрика (растёт сверх порога)."""
        history_list = getattr(self.history, metric, None)
