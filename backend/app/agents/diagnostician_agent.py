...
class DiagnosticianAgent(BaseAgent):
    ...
    async def _determine_root_cause(
        ...
        """Определяет root cause на основе корреляций и истории."""
        ...
        if strong_correlations:
            ...
            # Определяем root cause на основе корреляции
        ...
        # 2. Анализ похожих кейсов из RAG
        ...
        # 3. Эвристики на основе типа проблемы
        if "HFR" in problem and "вырос" in problem:
            if not strong_correlations:
                # Если нет сильных корреляций, предполагаем дрейф фокуса по времени
