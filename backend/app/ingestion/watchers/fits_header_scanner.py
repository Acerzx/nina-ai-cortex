...
class FITSHeaderScanner(BaseFileWatcher):
    ...
    async def process_file(self, path: Path) -> None:
        ...
        ИСПРАВЛЕНО (v4.0 — проблема #15): async_fits_getheader вместо sync.
        """
        ...
        # 4. ИСПРАВЛЕНО: Асинхронное чтение FITS заголовков
        header_dict = await async_fits_getheader(path, ext=0)
        if header_dict is None:
            return

        # Формируем FITSHeaderReport
        from app.ingestion.parsers.fits_header import WCSData, FITSHeaderReport

        wcs = WCSData(
            crval1=header_dict.get("CRVAL1"),
            crval2=header_dict.get("CRVAL2"),
            cd1_1=header_dict.get("CD1_1"),
            cd1_2=header_dict.get("CD1_2"),
            cd2_1=header_dict.get("CD2_1"),
            cd2_2=header_dict.get("CD2_2"),
        )

        report = FITSHeaderReport(
            file_name=path.name,
            wcs=wcs if wcs.crval1 is not None else None,
            moon_angl=header_dict.get("MOONANGL"),
            sun_angle=header_dict.get("SUNANGLE"),
            filter_name=header_dict.get("FILTER"),
            exposure_time=header_dict.get("EXPTIME"),
            temperature=header_dict.get("CCD-TEMP") or header_dict.get("TEMPERAT"),
        )

        # 5. Обновляем справочный кэш
        if (
            report.wcs
