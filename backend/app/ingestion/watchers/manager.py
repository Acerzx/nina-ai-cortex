...
class WatcherManager:
    ...
    async def start(self):
        ...
        # ИСПРАВЛЕНО: динамический вызов sync/async start()
        for watcher in self.watchers:
            await self._start_watcher_safely(watcher)
    ...
    async def _start_watcher_safely(self, watcher: BaseFileWatcher) -> None:
        """
        Безопасно запускает watcher, определяя тип метода start() runtime.

        Поддерживает:
        - Синхронный start() (большинство watcher'ов из BaseFileWatcher)
        - Асинхронный async def start() (новый HocusFocusWatcher из Этапа 9)
        """
        start_method = getattr(watcher, "start", None)
        if start_method is None:
            logger.warning(
                f"Watcher {watcher.__class__.__name__} has no start() method"
            )
            return
        ...
        try:
            if inspect.iscoroutinefunction(start_method):
                # Асинхронный вариант (HocusFocusWatcher)
                await start_method()
                logger.debug(f"✅ {watcher_name} started (async)")
            else:
                # Синхронный вариант (BaseFileWatcher наследники)
                result = start_method()
                # Дополнительная защита: если вернулась корутина — ждём
                if inspect.iscoroutine(result):
                    await result
                logger.debug(f"✅ {watcher_name} started (sync)")
        except Exception as e:
            logger.error(f"❌ Failed to start {watcher_name}: {e}")
            raise
