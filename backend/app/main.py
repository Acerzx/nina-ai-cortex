...
async def lifespan(app: FastAPI):
    ...
    try:
        # 1. Останавливаем агентов (они могут использовать клиенты)
        ...
        # 2. Останавливаем менеджеры
        await mode_manager.stop()

        # 3. Закрываем broadcast и RAG
        await ws_broadcast_manager.stop()
        await rag_engine.close()

        # 4. Останавливаем watchers (включая InfluxDB provider и Prometheus scraper)
        await watcher_manager.stop()

        # 5. Закрываем LLM клиент в самом конце
        await llm_client.close()

        # 6. Даем время на закрытие всех соединений
...
async def api_root():
    ...
    return {
        ...
        "health": "/health",
        "endpoints": {
            "observatory_state": "/api/v1/observatory/state",
            "metrics": "/api/v1/metrics",
            "sequence_shadow": "/api/v1/sequence/shadow",
            "sequence_state": "/api/v1/sequence/state",
            "agents_status": "/api/v1/agents/status",
            "rag_search": "/api/v1/rag/search",
            "rag_stats": "/api/v1/rag/stats",
            "ws_stats": "/api/v1/ws/stats",
            "plugins": "/api/v1/plugins",
            "simulation_start": "/api/v1/simulation/start",
            "preflight": "/api/v1/safety/preflight",
        },
    }
...
async def get_metrics():
    ...
    - Prometheus (температура, RMS, mount position)
    - Session Metadata (HFR, FWHM, stars из последнего кадра)
    - InfluxDB (исторические тренды)
    - ObservatoryState (текущее состояние)

    Используется для:
    - Внешних систем мониторинга (Grafana, Prometheus)
    - Легковесных клиентов
    ...
    # Добавляем тренды для ключевых метрик
    trends = {}
...
async def get_metrics_history(
    ...
    limit: int = Query(100, ge=1, le=1000, description="Количество точек"),
):
    """
    Возвращает историю конкретной метрики.

    Args:
        metric: Имя метрики (hfr, fwhm, rms_ra, rms_dec, temperature, wind_speed, humidity)
        limit: Максимальное количество точек (1-1000)

    Returns:
        Список значений метрики с временными метками
    """
    ...
    # Генерируем временные метки (примерные, на основе интервала обновления)
    # В реальности можно хранить точные timestamps в ObservatoryState
    timestamps = []
    now = datetime.now()
    for i in range(len(limited_history)):
        # Предполагаем интервал ~3 секунды между точками
...
@app.get("/api/v1/metrics/export", tags=["Metrics"])
async def export_metrics_prometheus():
    """
    Экспортирует метрики в Prometheus text format.

    Используется для интеграции с внешним Prometheus сервером.
    ...
    # Camera metrics
    if metrics.get("camera_temp") is not None:
        lines.append(
            f"# HELP nina_camera_temperature_celsius Camera sensor temperature"
        )
        lines.append(f"# TYPE nina_camera_temperature_celsius gauge")
        lines.append(f"nina_camera_temperature_celsius {metrics['camera_temp']}")

    # Image quality metrics
    if metrics.get("hfr") is not None:
        lines.append(f"# HELP nina_image_hfr_pixels Half Flux Radius")
        lines.append(f"# TYPE nina_image_hfr_pixels gauge")
        lines.append(f"nina_image_hfr_pixels {metrics['hfr']}")

    if metrics.get("fwhm") is not None:
        lines.append(f"# HELP nina_image_fwhm_pixels Full Width Half Maximum")
        lines.append(f"# TYPE nina_image_fwhm_pixels gauge")
        lines.append(f"nina_image_fwhm_pixels {metrics['fwhm']}")

    # Guider metrics
    if metrics.get("rms_ra") is not None:
        lines.append(f"# HELP nina_guider_rms_ra_arcsec Guider RMS in RA")
        lines.append(f"# TYPE nina_guider_rms_ra_arcsec gauge")
        lines.append(f"nina_guider_rms_ra_arcsec {metrics['rms_ra']}")

    if metrics.get("rms_dec") is not None:
        lines.append(f"# HELP nina_guider_rms_dec_arcsec Guider RMS in Dec")
        lines.append(f"# TYPE nina_guider_rms_dec_arcsec gauge")
        lines.append(f"nina_guider_rms_dec_arcsec {metrics['rms_dec']}")

    # Mount metrics
    if metrics.get("mount_altitude") is not None:
        lines.append(f"# HELP nina_mount_altitude_degrees Mount altitude")
        lines.append(f"# TYPE nina_mount_altitude_degrees gauge")
        lines.append(f"nina_mount_altitude_degrees {metrics['mount_altitude']}")

    if metrics.get("mount_azimuth") is not None:
        lines.append(f"# HELP nina_mount_azimuth_degrees Mount azimuth")
        lines.append(f"# TYPE nina_mount_azimuth_degrees gauge")
        lines.append(f"nina_mount_azimuth_degrees {metrics['mount_azimuth']}")

    # Weather metrics
    if weather.get("temperature") is not None:
        lines.append(f"# HELP nina_wx_temperature_celsius Ambient temperature")
        lines.append(f"# TYPE nina_wx_temperature_celsius gauge")
        lines.append(f"nina_wx_temperature_celsius {weather['temperature']}")

    if weather.get("humidity") is not None:
        lines.append(f"# HELP nina_wx_humidity_percent Relative humidity")
        lines.append(f"# TYPE nina_wx_humidity_percent gauge")
        lines.append(f"nina_wx_humidity_percent {weather['humidity']}")

    if weather.get("wind_speed") is not None:
        lines.append(f"# HELP nina_wx_wind_speed_mps Wind speed")
        lines.append(f"# TYPE nina_wx_wind_speed_mps gauge")
        lines.append(f"nina_wx_wind_speed_mps {weather['wind_speed']}")

    # Safety status (0 = SAFE, 1 = UNSAFE, -1 = UNKNOWN)
    safety_value = {"SAFE": 0, "UNSAFE": 1, "UNKNOWN": -1}.get(
        observatory_state.safety_status, -1
    )
    lines.append(
        f"# HELP nina_safety_status Safety monitor status (0=SAFE, 1=UNSAFE, -1=UNKNOWN)"
    )
    lines.append(f"# TYPE nina_safety_status gauge")
    lines.append(f"nina_safety_status {safety_value}")
